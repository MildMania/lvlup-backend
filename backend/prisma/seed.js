...
async function main() {
    ...
    const userBatches = await Promise.all(games.map(async (game, gameIndex) => {
        const users = [];
    ...
    for (const user of allUsers) {
        ...
        for (let i = 0; i < sessionsCount; i++) {
            const sessionStart = new Date(user.createdAt.getTime() + Math.random() * (Date.now() - user.createdAt.getTime()));
            ...
            const gameEvents = eventTypes[user.game.name] || eventTypes['Puzzle Quest Adventures'];
            const eventsCount = Math.floor(Math.random() * 15) + 5; // 5-20 events per session
            for (let j = 0; j < eventsCount; j++) {
                const eventTime = new Date(sessionStart.getTime() + (j / eventsCount) * sessionDuration * 1000);
    ...
    for (const game of games) {
        const gameCheckpoints = checkpointData[game.name] || [];
    ...
    for (const user of allUsers) {
        const userCheckpoints = allCheckpoints.filter((cp) => cp.gameId === user.gameId);
        ...
        for (const checkpoint of userCheckpoints.sort((a, b) => (a.order || 0) - (b.order || 0))) {
            if (Math.random() < completionRate) {
                const completionTime = new Date(user.createdAt.getTime() + Math.random() * (Date.now() - user.createdAt.getTime()));
