...
const MonetizationTab: React.FC<{ gameInfo: any }> = ({ gameInfo }) => {
  ...
  const fetchMonetizationCohorts = async () => {
    setLoading(true);
    try {
      const params = new URLSearchParams({
        gameId: gameInfo.id,
        startDate: filters.startDate,
        endDate: filters.endDate,
        maxDays: Math.max(...selectedDays).toString()
      });
      ...
      const response = await apiClient.get(`/analytics/metrics/monetization-cohorts?${params.toString()}`);
      ...
      console.error('Error fetching monetization cohorts:', error);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="analytics-filters" style={{ marginBottom: '20px' }}>
        <div className="filter-group">
          ...
          <select
            ...
            style={{ padding: '8px', borderRadius: '4px', border: '1px solid #ddd' }}
          >
            <option value="totalRevenue">Revenue (Total)</option>
                    ...
                    {selectedDays.map(day => {
                      ...
                      return (
                        <td key={day} className={`retention-cell ${colorClass}`}>
                          <div className="metric-value">{formattedValue}</div>
                          {!isNotAvailable && userCount !== undefined && (
                            <div className="metric-user-count">
                              {userCount} users
                              {selectedMetric === 'arppuIap' && payingUsers !== undefined && (
                                <span> ({payingUsers} paying)</span>
                              )}
                            </div>
                          )}
                        </td>
